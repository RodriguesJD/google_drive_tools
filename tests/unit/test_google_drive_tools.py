...
def test_list_domain_folders_by_searching_drives():
    list_domain_folders = drive_tools.list_domain_folders_by_searching_drives()
    assert isinstance(list_domain_folders, list)
...
def test_get_domain_folder_by_id_by_searching_drive():
    # TODO i don't own the domain folder so it is subject to change
    file_id_that_exists = os.environ["G_DRIVE_DOMAIN_TEST_DRIVE_ID"]
    folder = drive_tools.get_domain_folder_by_id_by_searching_files(file_id_that_exists)
    assert isinstance(folder, dict)
...
def test_dir_service():
    assert str(type(drive_tools.sheets_service())) == "<class 'googleapiclient.discovery.Resource'>"
...
def test_write_to_existing_sheet():
    ...
    result = drive_tools.write_to_existing_sheet(sheet_id, what_to_write_to_sheet)
